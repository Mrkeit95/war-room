...
type Kpi = { label: string; value: string; meta: string; color?: string; segment?: string }
type PipelineStage = { num: number; label: string; segment?: string }
        ...
        {kpis.map(k => {
          const card = (
            <div style={{
              ...
              cursor: k.segment ? 'pointer' : 'default',
              height: '100%',
            }}>
          ...
          return k.segment ? (
            <SegmentLink key={k.label} segment={k.segment} block>{card}</SegmentLink>
          ) : (
          ...
          {pipeline.map(s => {
            const tile = (
              <div style={{
                ...
                cursor: s.segment ? 'pointer' : 'default', height: '100%',
              }}>
            ...
            return s.segment ? (
              <SegmentLink key={s.label} segment={s.segment} block>{tile}</SegmentLink>
            ) : (
        ...
        <Panel title="Grade distribution" style={{ marginBottom: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {gradeDistribution.map(g => (
              <SegmentLink key={g.grade} segment={`${regionCode}:grade-${g.grade}`} block>
                <div style={{
                  background: 'var(--surface-2)', border: '1px solid var(--border)',
